...
#' @param repeats an integer giving the number of times to repeat `c(n1, n2)`. Only used
#' for applicable study designs that have more than two explanatory variables containing
#' the same sample sizes specified by `n1` and `n2`, else default setting is `repeats = 1`.
#' @param u a scalar or vector to evaluate \deqn{u'\beta > C,}
#' where \eqn{\beta} is an unknown parameter that is to be estimated.
#' Default setting is \eqn{u = 1.}
...
#' specifically designed under the normal linear regression model \deqn{yn = Xn\beta + \epsilon,}
#' \deqn{\epsilon ~ N(0, \sigma^2 Vn).} When set to `NULL`, \eqn{Xn} is generated in-function using
#' `bayesassurance::gen_Xn()`. Note that setting `Xn = NULL` also enables user to pass in a
...
#' @param surface_plot when set to `TRUE` and \eqn{n1} and \eqn{n2} are vectors, a contour map showcasing
#' various assurance values corresponding to different combinations of \eqn{n1} and \eqn{n2} is produced.
...
#' }
#' @examples ## Example
#' ## Vectors of sample sizes are passed in for both n1 and n2 to evaluate
#' ## assurance. Since we are passing in vectors for n1 and n2, we can
#' ## output a contour plot by evaluating unique combinations of the n1
#' ## and n2 sample sizes provided.
#'
#' n1 <- seq(20, 75, 5)
#' n2 <- seq(50, 160, 10)
#'
...
bayes_sim_unbalanced <- function(n1, n2, repeats = 1, u, C, Xn = NULL, Vn = NULL,
  ...
  MC_samp <- function(n1 = n1, n2 = n2){
    ...
    L_tran <- chol(Vbeta_a_inv + t(Xn) %*% Vn_inv %*% Xn) # note Vn was originally identity
  ...
  if(surface_plot == TRUE){
    ...
    assurance <- matrix(data=assurance, nrow=dim(Mesh_grid$x)[1], ncol=dim(Mesh_grid$x)[1], byrow = FALSE)

    assur_contourplot <- plotly::plot_ly(x = Mesh_grid$x[,1], y = Mesh_grid$y[1,], z = ~assurance, colorscale = list(c(0, 0.25, 0.5, 0.75, 1), c('white', 'lightcyan', 'lightskyblue', 'steelblue', 'blue'))) %>%
