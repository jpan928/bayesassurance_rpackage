...
#' @param n_a sample size at analysis stage. Also quantifies the amount of prior information we have for
#' parameter \eqn{\mu}.
#' @param n_d sample size at design stage. Also quantifies the amount of prior information we have
...
bayes_adcock <- function(n, d, mu_beta_a, mu_beta_d, n_a, n_d, sig_sq, alpha, mc_iter = 1000){
  ...
  if(length(n) > 1){
    assur_plot <- ggplot2::ggplot(assur_tab, alpha = 0.5, aes(x = .data$n, y = .data$Assurance)) +
      ggplot2::geom_point(aes(x = .data$n, y = .data$Assurance), lwd = 1.2) + ggplot2::ggtitle("Assurance Values") +
