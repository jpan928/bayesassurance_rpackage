...
#' @importFrom ggplot2 ggplot aes theme element_text xlab ylab ggtitle
...
#' \deqn{\epsilon ~ N(0, \sigma^2 I_n),} where \eqn{I_n} is an \eqn{n} by \eqn{n} identity matrix.
...
#' @param u fixed scalar or vector of the same dimension as \eqn{\beta_0} and \eqn{\beta_1}
...
#' \itemize{
#'      \item{rc_table:} table of sample size and corresponding correct classification rates
#'      \item{rc_plot:} plot of correct classification rates for varying sample sizes
#' }
#' @examples ## Example
#' n <- seq(100, 1200, 10)
...
bayes_goal_func <- function(n, Xn = NULL, K, pi, sigsq, u, beta_0, beta_1){
  ...
  if(dim(t(beta_0))[1] != dim(t(beta_1))[1] | dim(t(beta_0))[2] != dim(t(beta_1))[2]){
    stop("beta_0 and beta_1 must have equal dimensions.")
  }

  if(is.null(Xn) == FALSE){
    if(dim(t(beta_0))[2] != dim(t(Xn))[1]){
      stop("Dimension mismatch. Design matrix column dimension
         should match row dimensions of beta_0 and beta_1.")
    }

    if(is.scalar(beta_0) & dim(t(Xn))[1] & dim(u)[1] != 1){
      stop("Dimension mismatch. Design matrix column dimension
         should match row dimensions of beta_0 and beta_1.")
    }
  }
  ...
  rate.correct.class <- function(n = n){
    ...
    if(is.scalar(beta_0) & is.scalar(beta_1)){
      ...
      }else if(qr(t(Xn) %*% Xn)$rank < qr(as.matrix(cbind(t(Xn) %*% Xn, u)))$rank){
      ...
      omega_0 <- pnorm((((sqrt(sigsq)*sqrt(t(z) %*% z))/delta) * log((K * pi)/(1 - pi))) +
                         (delta/(2*sqrt(sigsq)*sqrt(t(z) %*% z))))

      omega_1 <- 1 - pnorm((((sqrt(sigsq)*sqrt(t(z) %*% z))/delta) * log((K * pi)/(1 - pi))) -
                         (delta/(2*sqrt(sigsq)*sqrt(t(z) %*% z))))
  ...
  if(length(n) > 1){
    assur_plot <- ggplot2::ggplot(assur_tab, alpha = 0.5, aes(x = .data$n,
      ...
      ggplot2::xlab("Sample Size n") + ggplot2::ylab("Rate of Correct Classification") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5))
  ...
  if(length(n) > 1){
    ...
    return(list(rc_val = paste0("Rate of Correct Classification: ", round(assur_tab$`Rate of Correct Classification`, 3))))
  }
