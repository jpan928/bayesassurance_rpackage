...
#' @param n_a sample size at analysis stage (setting n_a close to 0 corresponds to a weak analysis prior)
...
#' "less" tests if theta_1 < theta_0, and "two.sided" performs a two-sided test. alt = "greater"
#' by default.
#' @param alpha significance level
#' @param bayes_sim when set to "TRUE", this indicates that the user wants to include simulated assurance
#' results in the outputted plot. Default setting is "FALSE".
...
#'      values obtained from bayesassurance::assurance_nd_na().
...
#' (n_d set to be large) that results in the Bayesian assurance and frequentist
#' curve perfectly overlapping one another.
#' n <- seq(10, 200, 10)
#' n_a <- 1e-8
#' n_d <- 1e+8
#' theta_0 <- 0.15
...
#' out <- bayesassurance::pwr_curve(n = n, n_a = n_a, n_d = n_d, theta_0 = theta_0,
#' theta_1 = theta_1, sigsq = sigsq, alt = "greater", alpha = alpha, bayes_sim = TRUE, mc_iter = 5000)
#'
...
#' theta_1 = theta_1, sigsq = sigsq, alt = "greater", alpha = alpha, bayes_sim = TRUE, mc_iter = 5000)
#' @seealso \code{\link{ggplot2}}, \code{\link{pwr_freq}} for frequentist power function and
#' \code{\link{bayes_sim}} for the Bayesian assurance function
#' @export


pwr_curve <- function(n, n_a, n_d, theta_0, theta_1, sigsq, alt = "greater", alpha, bayes_sim = FALSE, mc_iter = NULL){

  if(length(n) <= 1){
    stop("n needs to be a vector with length greater than 1.")
  }

  # obtained the power and assurance curves to include in plot
  y1 <- bayesassurance::pwr_freq(n = n, theta_0 = theta_0, theta_1 = theta_1, sigsq = sigsq, alt = alt, alpha = alpha)
  y2 <- bayesassurance::assurance_nd_na(n = n, n_a = n_a, n_d = n_d, theta_0 = theta_0,
                                        theta_1 = theta_1, sigsq = sigsq, alt = alt, alpha = alpha)
  ...
  p <- ggplot2::ggplot(df1, alpha = 0.5, ggplot2::aes(x = .data$n, y = round(.data$Power, 3), color="Power")) +
    ggplot2::geom_line(aes(x = .data$n, y = .data$Power), lwd = 1.2)
  p1 <- p + ggplot2::geom_point(data = df2, alpha = 0.5, ggplot2::aes(x = .data$n, y = round(.data$Assurance, 3), color="Assurance"),lwd=1.2)
  pwr_curves <- p1 + ggplot2::xlab("Sample Size (n)") + ggplot2::ylab("Power/Assurance") +
    ggplot2::ggtitle("Power Curves of Frequentist and Bayesian Methods")

  # checks for bayes_sim option to see if simulated assurance points should also be included
  if(bayes_sim == TRUE){
    if(is.null(mc_iter)){
      mc_iter = 5000
    }
    y3 <- bayesassurance::bayes_sim(n, p = 1, u = 1, C = theta_0, Xn = NULL,
                                    ...
                                    mu_beta_a = 0, alt= alt, alpha = 0.05, mc_iter = mc_iter)
    df3 <- y3$assurance_table
    colnames(df3) <- c("n", "Assurance")
    p3 <- pwr_curves + ggplot2::geom_point(data = df3, alpha = 0.5, ggplot2::aes(x = .data$n, y = round(.data$Assurance, 3),
                                                               color = "Simulated Assurance"))
    pwr_curves <- p3 + ggplot2::xlab("Sample Size (n)") + ggplot2::ylab("Power/Assurance") +
    ggtitle("Power Curves of Frequentist and Bayesian Methods")

    pwr_curves<- structure(pwr_curves, class = "ggplot")

    return(list(power_table = df1, assurance_table = df2, bayes_sim_table = df3, mc_samples = mc_iter,
                plot = pwr_curves))
  }else{
    pwr_curves<- structure(pwr_curves, class = "ggplot")
