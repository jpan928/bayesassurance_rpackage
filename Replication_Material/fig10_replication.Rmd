...
p2 <- p1 + geom_point(data = df2, alpha = 0.5, aes(x = n, y = assur_vals, color = "Bayesian Assurance"), lwd = 1.2) +
...
p2 <- p2 + geom_label(aes(95, 1.03, label="~alpha[1] == ~beta[1] == 0.5~and~alpha[2] == ~beta[2] == 0.5"), parse=TRUE, color = "black", size = 2.5) + ylim(0.6, 1.03) + xlim(60, 350)
p2
```
...
p4 <- p4 + geom_label(aes(95, 0.95, label="~alpha[1] == ~beta[1] == 2~and~alpha[2] == ~beta[2] == 6"), parse=TRUE,color = "black", size = 2.5)
p4
...
p7 <- p6 + geom_line(data = df1, alpha = 0.5, aes(x = n, y = power_vals, color="Frequentist Power"), lwd = 1.2)

p8 <- p7 + geom_point(data = df2, alpha = 0.5, aes(x = n, y = assur_vals, color="Bayesian Assurance"),lwd=1.2)
p8 <- p8 + geom_label(
  aes(95, 0.72, label="~alpha[1] == ~beta[1] == 4~and~alpha[2] == ~beta[2] == 8"), parse=TRUE,
